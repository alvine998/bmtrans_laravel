...
<body class="bg-bm-cream min-h-screen flex items-center justify-center p-4">
  <div class="w-full max-w-[420px]">
    <div class="hazard-stripe"></div>
    <div class="bg-bm-cream-soft border border-bm-dark/10 p-8">
      ...
      @if($errors->any())
      <div class="mt-6 bg-bm-red/10 border border-bm-red/30 p-3 text-[13px] text-red-800">
      ...
      <form method="POST" action="{{ route('admin.login.post') }}" class="mt-8 space-y-5">
        ...
        <div>
          ...
          <input type="email" name="email" value="{{ old('email') }}" required autofocus class="mt-2 w-full bg-bm-cream border border-bm-dark/10 px-4 py-3 text-[14px] focus:border-bm-yellow outline-none" placeholder="admin@berkahmakmur.co.id">
        </div>
        <div>
          <label class="font-mono text-[11px] uppercase tracking-widest">Password</label>
          <input type="password" name="password" required class="mt-2 w-full bg-bm-cream border border-bm-dark/10 px-4 py-3 text-[14px] focus:border-bm-yellow outline-none" placeholder="••••••••••">
        </div>
