  ...
  <div class="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8 py-12 lg:py-20">
    <div class="grid lg:grid-cols-[1fr_1.1fr] gap-12">
      ...
      <div>
        ...
        <div class="mt-8 space-y-4">
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="border border-bm-black/10 p-4"><div class="font-mono text-[10px] uppercase">Jakarta</div><div class="font-display text-[14px] uppercase mt-1">Cakung, Jaktim — Transit hub</div></div>
            <div class="border border-bm-black/10 p-4"><div class="font-mono text-[10px] uppercase">Surabaya</div><div class="font-display text-[14px] uppercase mt-1">Margomulyo — Gudang 2.000m²</div></div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-bm-black-soft border border-white/10 text-white p-6 sm:p-8">
        ...
        @if($errors->any())
          <div class="mt-4 bg-bm-red/10 border border-bm-red/30 p-3 text-[13px] text-red-200">
        ...
        <form method="POST" action="{{ route('contact.store') }}" class="mt-6 space-y-4">
          ...
          <div class="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <input name="name" value="{{ old('name') }}" required class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2.5 text-[14px] focus:border-bm-yellow outline-none" placeholder="Nama penanggung jawab">
            </div>
            <div>
              <label class="font-mono text-[11px] uppercase">Perusahaan</label>
              <input name="subject" value="{{ old('subject') }}" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2.5 text-[14px]" placeholder="PT / CV">
            </div>
          </div>
          ...
          <div class="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="email" name="email" value="{{ old('email') }}" required class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2.5 text-[14px]" placeholder="email@perusahaan.com">
            </div>
            <div>
              <label class="font-mono text-[11px] uppercase">No WA / HP</label>
              <input name="phone" value="{{ old('phone') }}" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2.5 text-[14px]" placeholder="+62...">
            </div>
          </div>
