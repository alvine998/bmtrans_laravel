...
<form method="POST" action="{{ route('admin.gallery.store') }}" enctype="multipart/form-data" class="mt-6 bg-bm-black-soft border border-white/10 p-5 space-y-4 max-w-[560px]">
  @csrf
  <div><label class="font-mono text-[11px] uppercase">Judul</label><input name="title" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2 text-[13px]"></div>
  <div class="grid grid-cols-2 gap-3">
    <div><label class="font-mono text-[11px] uppercase">Tipe</label><select name="type" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2 text-[13px]"><option value="image">Image (WebP re-encode)</option><option value="video">Video (mp4 max 20MB)</option></select></div>
