...
<div class="mt-6 bg-bm-black-soft border border-white/10">
  <div class="grid grid-cols-12 gap-2 px-4 py-2 border-b border-white/5 font-mono text-[10px] uppercase text-bm-gray-light">
  ...
  @foreach($pages as $p)
    <div class="grid grid-cols-12 gap-2 px-4 py-3 border-b border-white/5 last:border-0 items-center hover:bg-white/[0.02]">
      <div class="col-span-3 font-mono text-[12px]"><span class="bg-white/10 px-2 py-0.5">{{ $p->slug }}</span></div>
      ...
      <div class="col-span-2"><a href="{{ route('admin.pages.edit',$p) }}" class="bg-bm-yellow text-bm-black px-3 py-1.5 font-mono text-[11px] uppercase font-bold">Edit wording →</a></div>
    </div>
  @endforeach
</div>
