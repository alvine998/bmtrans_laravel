...
<div class="flex items-start justify-between gap-4">
  <div>
    <a href="{{ route('admin.pages.index') }}" class="font-mono text-[11px] uppercase text-bm-gray-light hover:text-bm-dark">← Kembali ke Konten Halaman</a>
...
<form method="POST" action="{{ route('admin.pages.update',$page) }}" class="mt-6 grid lg:grid-cols-[1.3fr_0.7fr] gap-6" id="page-form">
  ...
  <div class="space-y-5">
    <div class="bg-bm-cream-soft border border-bm-dark/10 p-5">
      ...
      <div class="mt-4 grid sm:grid-cols-2 gap-4">
        <div>
          ...
          <input name="title" value="{{ old('title',$page->title) }}" required class="mt-1 w-full bg-bm-cream border border-bm-dark/10 px-3 py-2 text-[14px]">
        </div>
        <div>
          <label class="font-mono text-[11px] uppercase">Slug (jangan ganti sembarang)</label>
          <input name="slug" value="{{ old('slug',$page->slug) }}" class="mt-1 w-full bg-bm-cream border border-bm-dark/10 px-3 py-2 text-[13px] font-mono">
        </div>
      </div>
    </div>
    ...
    @elseif(count($editable))
      <div class="bg-bm-cream-soft border border-bm-dark/10 p-5 space-y-3">
        ...
        @foreach($editable as $key => $label)
          <div>
            ...
            <input name="sections[{{ $key }}]" value="{{ old('sections.'.$key, $s[$key] ?? '') }}" class="mt-1 w-full bg-bm-cream border border-bm-dark/10 px-3 py-2 text-[13px]">
          </div>
        @endforeach
      </div>
    @else
      <div class="bg-bm-cream-soft border border-bm-dark/10 p-5 font-mono text-[12px] text-bm-gray-light">
  ...
  <div class="space-y-4">
    <div class="bg-bm-cream-soft border border-bm-dark/10 p-5">
      ...
      <div class="mt-4 space-y-3">
        <div><label class="font-mono text-[10px] uppercase">SEO Title</label><input name="seo_title" value="{{ old('seo_title',$page->seo_title) }}" class="mt-1 w-full bg-bm-cream border border-bm-dark/10 px-3 py-2 text-[13px]"></div>
        <div><label class="font-mono text-[10px] uppercase">SEO Description</label><textarea name="seo_description" rows="4" class="mt-1 w-full bg-bm-cream border border-bm-dark/10 px-3 py-2 text-[12px]">{{ old('seo_description',$page->seo_description) }}</textarea></div>
      </div>
    ...
    @if($page->slug === 'beranda')
    <div class="bg-bm-yellow text-bm-dark p-4">
