...
<form method="POST" action="{{ route('admin.settings.update') }}" enctype="multipart/form-data" class="mt-6 space-y-6 max-w-[720px]" @unless($canEdit) onsubmit="return false" @endunless>
  ...
  <div class="bg-bm-black-soft border border-white/10 p-5">
    ...
    <div class="mt-4 flex items-center gap-4">
      <div class="w-20 h-20 bg-bm-black border border-white/10 flex items-center justify-center overflow-hidden shrink-0">
        @if($logo)
        ...
        <input type="file" name="logo" accept="image/png,image/jpeg,image/webp" @disabled(! $canEdit) class="flex-1 text-[12px] font-mono file:mr-3 file:bg-bm-red file:text-white file:border-0 file:px-3 file:py-2 file:uppercase file:font-bold file:text-[11px] bg-bm-black border border-white/10 px-3 py-2 disabled:opacity-50">
    </div>
  </div>
          ...
          <div>
            ...
            <div class="mt-1 flex gap-2">
              ...
              @if(str_contains($s->key,'address') || str_contains($s->key,'description'))
                <textarea name="settings_data[{{ $formKey }}]" rows="2" @readonly(! $canEdit) class="w-full bg-bm-black border border-white/10 px-3 py-2 text-[13px] font-mono">{{ old('settings_data.'.$formKey, $s->value) }}</textarea>
              @else
                <input type="text" name="settings_data[{{ $formKey }}]" value="{{ old('settings_data.'.$formKey, $s->value) }}" @readonly(! $canEdit) class="w-full bg-bm-black border border-white/10 px-3 py-2 text-[13px] font-mono">
              @endif
            </div>
          </div>
        @endforeach
  ...
  <div class="bg-bm-yellow/10 border border-bm-yellow/20 p-4">
    ...
    <div class="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
      <div>
        ...
        <input name="new_key" placeholder="ex: contact.address" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2 text-[12px] font-mono">
      </div>
      <div>
        <label class="font-mono text-[10px] uppercase">Value baru</label>
        <input name="new_value" placeholder="Jl. Contoh No 123" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2 text-[12px]">
      </div>
    </div>
  </div>
