...
<form method="POST" action="{{ $service->exists ? route('admin.services.update',$service) : route('admin.services.store') }}" enctype="multipart/form-data" class="mt-6 grid lg:grid-cols-[1.2fr_0.8fr] gap-6" id="service-form">
  ...
  <div class="space-y-4 bg-bm-cream-soft border border-bm-dark/10 p-5">
    <div>
      ...
      <input name="title" value="{{ old('title',$service->title) }}" required class="mt-1 w-full bg-bm-cream border border-bm-dark/10 px-3 py-2 text-[14px]">
    </div>
    <div>
      <label class="font-mono text-[11px] uppercase">Slug (otomatis jika kosong)</label>
      <input name="slug" value="{{ old('slug',$service->slug) }}" class="mt-1 w-full bg-bm-cream border border-bm-dark/10 px-3 py-2 text-[13px] font-mono">
    </div>
    <div>
      <label class="font-mono text-[11px] uppercase">Excerpt 1 baris</label>
      <input name="excerpt" value="{{ old('excerpt',$service->excerpt) }}" class="mt-1 w-full bg-bm-cream border border-bm-dark/10 px-3 py-2 text-[13px]">
    </div>
    <div>
      <label class="font-mono text-[11px] uppercase">Body (HTML disanitasi via Purifier)</label>
      <textarea name="body" rows="10" class="mt-1 w-full bg-bm-cream border border-bm-dark/10 px-3 py-2 text-[13px]">{{ old('body',$service->body) }}</textarea>
    ...
    <div>
      ...
      @endphp
      <textarea name="features_text" id="features_text" rows="5" class="mt-1 w-full bg-bm-cream border border-bm-dark/10 px-3 py-2 text-[13px] font-mono" placeholder="Armada GPS real-time&#10;Asuransi all-risk">{{ $feat }}</textarea>
  ...
  <div class="space-y-4">
    <div class="bg-bm-cream-soft border border-bm-dark/10 p-5">
      <div class="grid grid-cols-2 gap-3">
        <div><label class="font-mono text-[11px] uppercase">Order</label><input type="number" name="order" value="{{ old('order',$service->order ?? 0) }}" class="mt-1 w-full bg-bm-cream border border-bm-dark/10 px-3 py-2"></div>
      ...
      <div class="mt-4">
        ...
        <input type="file" name="image" accept="image/*" class="mt-1 w-full bg-bm-cream border border-bm-dark/10 px-3 py-2 text-[12px]">
      ...
      <div class="mt-4">
        ...
        <input name="seo_title" value="{{ old('seo_title',$service->seo_title) }}" class="mt-1 w-full bg-bm-cream border border-bm-dark/10 px-3 py-2 text-[13px]">
      </div>
      <div class="mt-2">
        <label class="font-mono text-[11px] uppercase">SEO Description</label>
        <textarea name="seo_description" rows="3" class="mt-1 w-full bg-bm-cream border border-bm-dark/10 px-3 py-2 text-[12px]">{{ old('seo_description',$service->seo_description) }}</textarea>
      </div>
