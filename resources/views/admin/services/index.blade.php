...
<div class="flex items-center justify-between">
  ...
  <a href="{{ route('admin.services.create') }}" class="bg-bm-yellow text-bm-dark px-4 py-2 font-display uppercase text-[12px] font-bold">+ Tambah Layanan</a>
</div>
...
<div class="mt-6 bg-bm-cream-soft border border-bm-dark/10 overflow-x-auto">
  <table class="w-full text-[13px]">
    <thead class="font-mono text-[11px] uppercase text-bm-gray-light border-b border-bm-dark/5"><tr><th class="text-left p-3">Order</th><th class="text-left p-3">Judul</th><th class="text-left p-3">Slug</th><th class="text-left p-3">Aktif</th><th class="text-left p-3">Aksi</th></tr></thead>
    <tbody>
      @foreach($services as $s)
        <tr class="border-b border-bm-dark/5 hover:bg-bm-dark/[0.02]">
          ...
          <td class="p-3"><span class="px-2 py-1 text-[10px] {{ $s->is_active ? 'bg-bm-yellow text-bm-dark' : 'bg-bm-dark/10' }}">{{ $s->is_active ? 'ACTIVE' : 'OFF' }}</span></td>
          <td class="p-3 flex gap-2">
            <a href="{{ route('admin.services.edit',$s) }}" class="px-3 py-1 border border-bm-dark/10 hover:bg-bm-dark/5">Edit</a>
