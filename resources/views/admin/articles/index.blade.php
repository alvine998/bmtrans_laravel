...
<div class="flex items-center justify-between">
  ...
  <a href="{{ route('admin.articles.create') }}" class="bg-bm-yellow text-bm-black px-4 py-2 font-display uppercase text-[12px] font-bold">+ Artikel</a>
</div>
...
<div class="mt-6 bg-bm-black-soft border border-white/10 overflow-x-auto">
  <table class="w-full text-[13px]">
    <thead class="font-mono text-[11px] uppercase text-bm-gray-light border-b border-white/5"><tr><th class="p-3 text-left">Judul</th><th class="p-3">Kategori</th><th class="p-3">Status</th><th class="p-3">Tgl</th><th class="p-3">Aksi</th></tr></thead>
    <tbody>
      @foreach($articles as $a)
        <tr class="border-b border-white/5">
          ...
          <td class="p-3"><span class="px-2 py-1 text-[10px] {{ $a->status==='published' ? 'bg-bm-yellow text-bm-black' : 'bg-white/10' }}">{{ $a->status }}</span></td>
          <td class="p-3 font-mono text-[11px]">{{ $a->published_at?->format('d/m/Y') }}</td>
          <td class="p-3 flex gap-2"><a href="{{ route('admin.articles.edit',$a) }}" class="px-3 py-1 border border-white/10">Edit</a>
