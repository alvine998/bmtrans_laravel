...
<form method="POST" action="{{ $partner->exists ? route('admin.partners.update',$partner) : route('admin.partners.store') }}" enctype="multipart/form-data" class="mt-6 grid lg:grid-cols-[1.2fr_0.8fr] gap-6">
  ...
  <div class="space-y-4 bg-bm-black-soft border border-white/10 p-5">
    <div>
      ...
      <input name="name" value="{{ old('name',$partner->name) }}" required class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2 text-[14px]">
    </div>
    <div>
      <label class="font-mono text-[11px] uppercase">Slug (otomatis jika kosong)</label>
      <input name="slug" value="{{ old('slug',$partner->slug) }}" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2 text-[13px] font-mono">
    </div>
    <div>
      <label class="font-mono text-[11px] uppercase">URL (tautan ke website partner)</label>
      <input name="url" value="{{ old('url',$partner->url) }}" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2 text-[13px]" placeholder="https://">
    </div>
  </div>
  ...
  <div class="space-y-4">
    <div class="bg-bm-black-soft border border-white/10 p-5">
      <div class="grid grid-cols-2 gap-3">
        <div><label class="font-mono text-[11px] uppercase">Order</label><input type="number" name="order" value="{{ old('order',$partner->order ?? 0) }}" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2"></div>
      ...
      <div class="mt-4">
        ...
        <input type="file" name="logo" accept="image/*" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2 text-[12px]">
        @if($partner->logo)<div class="mt-2"><img src="{{ asset('storage/'.$partner->logo) }}" alt="" class="h-12 border border-white/10"><div class="mt-1 font-mono text-[10px] text-bm-gray-light">{{ $partner->logo }}</div></div>@endif
      </div>
