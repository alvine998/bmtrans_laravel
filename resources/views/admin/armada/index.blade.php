...
<div class="flex items-center justify-between">
  ...
  <a href="{{ route('admin.armada.create') }}" class="bg-bm-yellow text-bm-black px-4 py-2 font-display uppercase text-[12px] font-bold">+ Tambah Armada</a>
</div>
    ...
    <tbody>
      @foreach($armadas as $a)
        <tr class="border-b border-white/5 hover:bg-white/[0.02]">
          ...
          <td class="p-3"><span class="inline-flex items-center gap-1 bg-bm-yellow text-bm-black px-2 py-1 font-display font-bold text-[11px]">{{ $a->price_note }} Rp {{ $a->display_price }}</span></td>
          <td class="p-3"><span class="px-2 py-1 text-[10px] {{ $a->is_active ? 'bg-bm-yellow text-bm-black' : 'bg-white/10' }}">{{ $a->is_active ? 'ACTIVE' : 'OFF' }}</span></td>
          <td class="p-3 flex gap-2">
            <a href="{{ route('admin.armada.edit',$a) }}" class="px-3 py-1 border border-white/10 hover:bg-white/5">Edit</a>
