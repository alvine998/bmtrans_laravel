...
<form method="POST" action="{{ $armada->exists ? route('admin.armada.update',$armada) : route('admin.armada.store') }}" enctype="multipart/form-data" class="mt-6 grid lg:grid-cols-[1.2fr_0.8fr] gap-6">
  ...
  <div class="space-y-4 bg-bm-black-soft border border-white/10 p-5">
    <div>
      ...
      <input name="name" value="{{ old('name',$armada->name) }}" required class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2 text-[14px]" placeholder="Pickup Bak">
    </div>
    <div class="grid grid-cols-2 gap-3">
      <div>
        <label class="font-mono text-[11px] uppercase">Tipe / Group (untuk filter)</label>
        <input name="type" value="{{ old('type',$armada->type) }}" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2 text-[13px] font-mono" placeholder="pickup / colt_diesel / fusso / tronton">
      ...
      <div>
        ...
        <input type="number" name="order" value="{{ old('order',$armada->order ?? 0) }}" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2">
      </div>
    </div>
    ...
    <div class="border border-bm-yellow/20 bg-bm-yellow/5 p-4">
      ...
      <div class="grid grid-cols-2 gap-3 mt-3">
        <div>
          ...
          <input type="number" name="price_start" value="{{ old('price_start',$armada->price_start ?? 0) }}" required min="0" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2 text-[14px] font-mono" placeholder="200000">
        ...
        <div>
          ...
          <input name="price_label" value="{{ old('price_label',$armada->price_label) }}" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2 text-[14px] font-mono" placeholder="200rb — kosongkan = auto">
      ...
      <div class="mt-3">
        ...
        <input name="price_note" value="{{ old('price_note',$armada->price_note ?? 'Mulai dari') }}" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2 text-[13px]" placeholder="Mulai dari">
      </div>
      @if($armada->exists)
        <div class="mt-3 font-mono text-[11px]">Preview: <span class="bg-bm-yellow text-bm-black px-2 py-1 font-bold">{{ $armada->price_note }} Rp {{ $armada->display_price }}</span></div>
      @endif
    </div>

    <div>
      <label class="font-mono text-[11px] uppercase">Deskripsi / Kapasitas</label>
      <textarea name="description" rows="3" class="mt-1 w-full bg-bm-black border border-white/10 px-3 py-2 text-[13px]" placeholder="Kapasitas 2 ton, cocok dalam kota...">{{ old('description',$armada->description) }}</textarea>
    </div>
  </div>
  ...
  <div class="space-y-4">
    <div class="bg-bm-black-soft border border-white/10 p-5">
      ...
      <div class="mt-5">
        ...
        <input id="armada-image-input" type="file" name="image" accept="image/jpeg,image/png,image/webp" class="mt-2 w-full bg-bm-black border border-white/10 px-3 py-2 text-[12px] file:bg-bm-yellow file:text-bm-black file:border-0 file:px-3 file:py-1 file:font-mono file:text-[11px] file:uppercase file:font-bold file:mr-3 file:cursor-pointer">
        <div id="armada-image-preview" class="mt-3 hidden">
          ...
          <img id="armada-preview-img" class="w-full h-[180px] object-cover border border-bm-yellow/30 bg-bm-black">
        </div>

        @if($armada->image)
          <div class="mt-4 border border-white/10 p-2 bg-bm-black">
            <div class="flex items-center justify-between">
              <span class="font-mono text-[10px] uppercase text-bm-gray-light">Foto tersimpan saat ini</span>
              <span class="font-mono text-[9px] px-1.5 py-0.5 bg-bm-yellow text-bm-black">ACTIVE IMAGE</span>
            </div>
            <img src="{{ $armada->image_url }}" class="mt-2 w-full h-[160px] object-cover border border-white/10 bg-bm-black">
        ...
        <div class="mt-3 p-2.5 border border-dashed border-white/10 bg-bm-black/50">
      ...
      <div class="mt-4 bg-bm-black border border-white/10 p-3 font-mono text-[10px] text-bm-gray-light">
        ...
        <div class="mt-2 bg-white/5 p-2 text-white">"Halo BM Trans, saya ingin negosiasi harga {{ $armada->name ?: '[Nama Armada]' }}. Bisa dibantu?"</div>
      </div>
    </div>
  </div>
</form>
