    ...
    <div class="flex items-end justify-between gap-6 flex-wrap">
      ...
      <div class="flex gap-2 font-mono text-[11px] uppercase">
        @foreach(['all'=>'Semua','fleet'=>'Armada','warehouse'=>'Gudang','operations'=>'Operasional'] as $k=>$lbl)
          <a href="{{ route('gallery',['category'=>$k]) }}" class="px-3 py-2 border {{ $category===$k ? 'bg-bm-yellow text-bm-black border-bm-yellow' : 'border-white/10 hover:bg-white/5' }}">{{ $lbl }}</a>
        @endforeach
      </div>
    </div>
    ...
    <div class="mt-10 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
      @forelse($items as $item)
        <div class="group relative bg-bm-black-soft border border-white/5 overflow-hidden">
          <div class="aspect-[4/3] bg-bm-black-light flex items-center justify-center">
            @if($item->type==='image')
      ...
      @empty
        <div class="col-span-full py-20 text-center border border-dashed border-white/10 font-mono text-bm-gray-light">Belum ada dokumentasi — tim lapangan akan upload setelah retur dari rute.</div>
      @endforelse
    </div>
