...
<section class="bg-bm-cream pt-10 pb-16">
  <div class="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8">
    @if($logo)
      <div class="w-16 h-16 bg-bm-cream-soft border border-bm-dark/10 flex items-center justify-center overflow-hidden mb-4">
    ...
    <div class="mt-12 grid lg:grid-cols-[1.1fr_0.9fr] gap-12">
      <div class="max-w-none">
        <p class="text-[18px] leading-relaxed text-bm-dark/80">{{ $intro1 }}</p>
        <p class="mt-4 text-bm-dark/70">{{ $intro2 }}</p>
        <h3 class="font-display uppercase mt-8 text-bm-yellow">Visi & Misi</h3>
        <ul class="mt-2 space-y-2 text-bm-dark/70">
        ...
        <div class="mt-2 grid grid-cols-2 gap-3 font-mono text-[11px]">
          <span class="border border-bm-dark/10 p-3">{{ $legalSiup }}</span>
          <span class="border border-bm-dark/10 p-3">{{ $legalNpwp }}</span>
          <span class="border border-bm-dark/10 p-3">{{ $legalTdp }}</span>
          <span class="border border-bm-dark/10 p-3 bg-bm-yellow text-bm-dark font-bold">{{ $legalIso }}</span>
        </div>
      </div>
        ...
        @if($team->count())
          <div class="bg-bm-cream-soft border border-bm-dark/10 p-6">
            ...
            <div class="mt-4 space-y-3">
              @foreach($team as $m)
                <div class="flex gap-3 p-3 border border-bm-dark/5">
