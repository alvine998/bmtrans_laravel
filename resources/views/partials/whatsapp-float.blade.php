...
<div id="wa-float" class="fixed bottom-5 right-5 md:bottom-7 md:right-7 z-[9999] flex items-center gap-3 pointer-events-none" style="isolation:isolate; transform: translateZ(0);">
  ...
  <div id="wa-label" class="pointer-events-auto hidden sm:flex items-center gap-2 bg-white border border-bm-dark/10 rounded-full pl-4 pr-3 py-2 shadow-[0_8px_24px_rgba(0,0,0,0.5)] translate-x-3 opacity-0 will-change-transform">
    ...
    <span class="font-display font-bold uppercase text-[12px] tracking-wide text-bm-dark whitespace-nowrap">Chat Admin — Balas &lt; 5 Menit</span>
    <span class="ml-1 w-6 h-6 bg-bm-yellow text-bm-dark rounded-full flex items-center justify-center text-[12px] font-bold">!</span>
  </div>

  {{-- Button --}}
  <a href="{{ $waLink }}" target="_blank" rel="noopener noreferrer"
     aria-label="Chat WhatsApp PT Berkah Makmur Transport"
     class="wa-btn pointer-events-auto group relative w-14 h-14 md:w-[60px] md:h-[60px] rounded-full bg-white border-2 border-bm-yellow flex items-center justify-center shadow-[0_12px_32px_rgba(0,0,0,0.45),0_0_0_6px_rgba(244,196,48,0.12)] hover:shadow-[0_16px_40px_rgba(0,0,0,0.6),0_0_0_8px_rgba(244,196,48,0.2)] hover:border-bm-dark hover:-translate-y-0.5 transition-all duration-300">
    {{-- hazard tick ring --}}
    <span class="absolute -inset-2 rounded-full opacity-60 group-hover:opacity-100 transition-opacity"
          style="background: repeating-conic-gradient(from 0deg, #A0A3A8 0 8deg, #6B6E73 8deg 16deg); -webkit-mask: radial-gradient(circle, transparent 30px, black 31px 36px, transparent 37px); mask: radial-gradient(circle, transparent 30px, black 31px 36px, transparent 37px);"></span>

    {{-- ping --}}
    <span class="absolute inset-0 rounded-full border border-bm-yellow/40 animate-ping [animation-duration:2.5s]"></span>
    <span class="absolute -top-1 -right-1 w-3 h-3 bg-bm-red border-2 border-bm-dark rounded-full animate-pulse"></span>

    {{-- WhatsApp icon — self-hosted SVG, no external lib --}}
    <svg class="relative w-7 h-7 text-bm-yellow group-hover:text-bm-cream transition-colors" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
