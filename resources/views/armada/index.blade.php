...
@section('content')
{{-- Hero — industrial --}}
<section class="relative bg-bm-black overflow-hidden border-b border-white/5">
...
{{-- Price list --}}
<section class="py-12 lg:py-16 bg-bm-white text-bm-black">
  <div class="mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8">

    @if($armadas->isEmpty())
      <div class="bg-bm-black text-white p-8 border border-white/10 max-w-[60ch]">
        <h2 class="font-display font-black text-[24px] uppercase">Daftar armada belum tersedia</h2>
        <p class="mt-2 text-[14px] text-bm-white/70">Admin sedang mengisi data armada. Hubungi WA untuk harga cepat.</p>
        <a href="https://wa.me/{{ $whatsapp }}" class="mt-4 inline-block bg-bm-yellow text-bm-black px-4 py-2 font-display uppercase text-[12px]">Chat WA</a>
      </div>
    @else

      {{-- Quick filter by type --}}
      <div class="flex flex-wrap items-center gap-2 pb-6 border-b border-black/10">
        <span class="font-mono text-[11px] uppercase tracking-widest font-bold">Filter:</span>
        <button data-filter="all" class="filter-btn is-active px-3 py-1.5 border border-black bg-black text-white font-mono text-[11px] uppercase">Semua ({{ $armadas->count() }})</button>
        @foreach($grouped->keys() as $type)
          <button data-filter="{{ \Illuminate\Support\Str::slug($type) }}" class="filter-btn px-3 py-1.5 border border-black/15 font-mono text-[11px] uppercase hover:bg-black hover:text-white">{{ $type }} ({{ $grouped[$type]->count() }})</button>
        @endforeach
      </div>
      ...
      <div class="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-6" id="armada-grid">
        @foreach($armadas as $a)
          ...
          <div data-type="{{ $typeSlug }}" class="armada-card group relative bg-white border border-black/10 p-0 flex flex-col overflow-hidden hover:border-bm-black/25 hover:shadow-[0_10px_40px_rgba(0,0,0,0.08)] transition-all duration-300">
            {{-- hazard accent --}}
            ...
            <div class="p-6 sm:p-7 flex flex-col flex-1">
              ...
              <div class="flex items-start justify-between gap-3">
                <div class="flex items-center gap-2">
                  <span class="font-mono text-[10px] uppercase tracking-widest px-2 py-1 bg-bm-black text-white">{{ $a->type ?: 'ARMADA' }}</span>
                  <span class="font-mono text-[10px] text-bm-gray-light">{{ sprintf('%02d', $loop->iteration) }}</span>
                </div>
                <span class="w-6 h-6 border border-black/10 flex items-center justify-center group-hover:bg-bm-black group-hover:text-white transition">↗</span>
              </div>

              <div class="mt-5 aspect-[16/10] bg-bm-black-light overflow-hidden border border-black/5">
                @if($img)
                ...
                @else
                  <div class="w-full h-full relative flex items-center justify-center">
                    <div class="font-display font-black text-[56px] leading-none text-black/[0.06] uppercase">{{ \Illuminate\Support\Str::limit($a->name,3,'') }}</div>
              ...
              <h3 class="mt-5 font-display font-black text-[26px] leading-[0.9] uppercase text-bm-black">{{ $a->name }}</h3>
              @if($a->description)
                <p class="mt-2 text-[13px] leading-relaxed text-bm-black/60 line-clamp-2">{{ $a->description }}</p>
              @endif

              {{-- Price block — industrial --}}
              <div class="mt-auto pt-5 flex items-end justify-between gap-3 border-t border-black/10">
                <div>
                  <div class="font-mono text-[10px] uppercase tracking-widest text-bm-black/50">{{ $a->price_note }}</div>
                  <div class="mt-1 flex items-baseline gap-1 text-bm-black">
                    <span class="font-mono text-[12px]">Rp</span>
                    <span class="font-display font-black text-[28px] leading-none">{{ $a->display_price }}</span>
                  </div>
                  <div class="mt-1 font-mono text-[10px] text-bm-black/50">negotiable • tarif rute menyesuaikan</div>
                </div>
...
(() => {
  ...
  btns.forEach(b => b.addEventListener('click', () => {
    ...
    btns.forEach(x => { x.classList.remove('is-active','bg-black','text-white'); x.classList.add('border-black/15'); });
    b.classList.add('is-active','bg-black','text-white');
    b.classList.remove('border-black/15');
    cards.forEach(c => {
